...
import { MongodbSettings, MongoDBPreparer } from "../Mongo";
import { AceBaseServerEmailSettings } from "./settings/email";
import { MongoDBTransaction, storageSettings } from "../Mongo/MongoDBTransaction";
...
type PrivateStorageSettings = AceBaseStorageSettings & {
	info?: string;
	type?: "data" | "transaction" | "auth" | "log";
};
...
export class SyncMongoServer extends SimpleEventEmitter {
	...
	constructor(readonly dbname: string, readonly options?: Partial<SyncMongoServerSettings>) {
		...
		if (this.config.auth.enabled && !this.config.https.enabled) {
			this.debug.warn(
				`WARNING: Authentication is enabled, but the server is not using https. Any password and other data transmitted may be intercepted!`.colorize(
					ColorStyle.red,
				),
			);
