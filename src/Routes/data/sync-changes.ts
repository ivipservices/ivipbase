...
import { sendBadRequestError, sendError, sendUnauthorizedError } from "../../lib/Errors";
...
export const addRoute = (env: RouteInitEnvironment) => {
	env.router.get(`/sync/changes/${env.db.name}`, async (req: Request, res) => {
		...
		try {
			...
			const targets =
				typeof data.path === "string" ? [{ path: data.path, events: ["value"] }] : typeof data.for === "string" ? (JSON.parse(data.for) as Array<{ path: string; events: string[] }>) : null;
			if (targets === null) {
