...
import { AccessCheckOperation, HasAccessResult } from "../../lib/Rules";
import { RouteInitEnvironment, RouteRequest } from "../../types";
import { sendUnauthorizedError } from "../../lib/Errors";

export type RequestQuery = { [key: string]: any; type: "info" | "children"; impersonate?: string };
...
export type ResponseBody = IReflectionNodeInfo &
...
export const addRoute = (env: RouteInitEnvironment) => {
	env.router.get(`/reflect/${env.db.name}/*`, async (req: Request, res) => {
		...
		const impersonatedAccess: Impersonated = {
		...
		try {
			...
			if (impersonatedAccess.uid) {
				...
				let list: Pick<IReflectionNodeInfo, "key" | "type" | "value" | "address" | "access">[];
				if (type === "children") {
					list = result.list;
				} else if (type === "info") {
