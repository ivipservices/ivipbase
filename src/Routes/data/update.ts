...
import { AccessRuleValidationError, RuleValidationFailCode } from "../../lib/Rules";
...
export const addRoute = (env: RouteInitEnvironment) => {
	env.router.post(`/data/${env.db.name}/*`, async (req: Request, res) => {
		...
		try {
			...
			await env.db.ref(path).context(req.context).update(val);
