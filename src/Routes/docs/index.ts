...
export const addRoute = (env: RouteInitEnvironment) => {
	...
	const options: swaggerJsdoc.Options = {
		definition: {
			...
			servers: [{ url: `http${env.config.https?.enabled ? "s" : ""}://${env.config.host}:${env.config.port}${env.config.rootPath ? `/${env.config.rootPath}` : ""}` }],
			tags: [
				{
					name: "auth",
