...
export const addMiddleware = (env: RouteInitEnvironment) => {
	...
	env.router.use<any>(async (req: RouteRequest<{ auth_token?: string }>, res, next) => {
		...
		if (typeof authorization === "string" && authorization.startsWith("Bearer ")) {
			...
			try {
				tokenDetails = decodePublicAccessToken(token, env.tokenSalt as any);
			} catch (err) {
				return sendNotAuthenticatedError(res, "invalid_token", "The passed token is invalid. Sign in again");
			}

			// Is this token cached?
			req.user = env.authCache?.get(tokenDetails.uid) ?? undefined;
			if (!req.user) {
