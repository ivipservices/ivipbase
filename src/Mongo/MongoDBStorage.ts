...
export class CustomStorage extends Storage {
	...
	private _storeNode(path: string, node: StorageNode, options: { transaction: CustomStorageTransaction }): void | Promise<void> {
	...
	private _processReadNodeValue(node: StorageNode) {
		...
		const start = async () => {
			...
			try {
				...
				await (async () => {
					...
					const addChildNode = (childPath: string, node: StorageNodeMetaData) => {
						if (!checkExecuted) {
							this.throwImplementationError(`childrenOf did not call checkCallback before addCallback`);
						}
						const key = PathInfo.get(childPath).key;
		...
		try {
			const node = await (async () => {
				...
				if (!targetNode) {
					// Lookup parent node
					if (path === "") {
						return { value: null } as StorageNode;
				 ...
				 */
				const applyFiltersOnInlineData = (descPath: string, node: StorageNode) => {
				...
				let checkExecuted = false;
				const includeDescendantCheck = (descPath: string, metadata: StorageNodeMetaData) => {
					checkExecuted = true;
					if (!transaction.production && !pathInfo.isAncestorOf(descPath)) {
						// Double check failed
						this.throwImplementationError(`"${descPath}" is not a descendant of "${path}" - descendantsOf must only check and return paths that are descendants`);
					...
					const checkPathInfo = new PathInfo(trailKeys);
					let include =
						(includeFilter.length > 0 ? includeFilter.some((info) => checkPathInfo.isOnTrailOf(info)) : true) &&
				...
				type DescendantRow = StorageNode & { path: string };
				const descRows = [] as DescendantRow[];
				const addDescendant = (descPath: string, node: StorageNode) => {
						...
						for (let j = 0; j < trailKeys.length; j++) {
							...
							if (key in parent) {
								...
								if (!mergePossible) {
									...
									this.debug.error(
										`The value stored in node "${otherNode.path}" cannot be merged with the parent node, value will be ignored. This error should disappear once the target node value is updated. See issue #20 for more information`,
										{ path, parent, key, nodeType, nodeValue },
									);
								} else {
									Object.keys(nodeValue).forEach((childKey) => {
										if (childKey in parent[key]) {
											this.throwImplementationError(
												`Custom storage merge error: child key "${childKey}" is in parent value already! Make sure the get/childrenOf/descendantsOf methods of the custom storage class return values that can be modified by AceBase without affecting the stored source`,
											);
										}
