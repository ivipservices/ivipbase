...
export function ColorsSupported() {
	...
	if (typeof process === "undefined" || !process.stdout || !process.env || !process.platform || (process.platform as string) === "browser") {
		return false;
	}
...
String.prototype.colorize = function (style: ColorStyle | ColorStyle[]) {
	return Colorize(this as string, style);
};
