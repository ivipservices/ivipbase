...
export type StartResponseBody =
	| ApiTransactionDetails // 200
...
export const addRoutes = (env: LocalServer) => {
	...
	env.router.post(`/transaction/:dbName/start`, async (req: StartRequest, res) => {
		...
		const access = await env.rules(dbName).isOperationAllowed(req.user ?? ({} as any), data.path, "transact");
		if (!access.allow) {
		...
		try {
			...
			const donePromise = env.db(dbName).storage.transaction(
				tx.path,
				async (val) => {
					env.debug.verbose(`Transaction ${tx.id} started with value: `, val);

					const access = await env.rules(dbName).isOperationAllowed(req.user ?? ({} as any), data.path, "get", { value: val, context: req.context });
					if (!access.allow) {
						env.log.error(LOG_ACTION, "unauthorized", { ...LOG_DETAILS, rule_code: access.code, rule_path: access.rulePath ?? null }, access.details);
	...
	env.router.post(`/transaction/:dbName/finish`, async (req: FinishRequest, res) => {
		const { dbName } = req.params;
		...
		try {
			...
			let access = await env.rules(dbName).isOperationAllowed(req.user ?? ({} as any), tx.path, "set");
			if (!access.allow) {
			...
			access = await env.rules(dbName).isOperationAllowed(req.user ?? ({} as any), tx.path, "set", { value: newValue, context: tx.context });
			if (!access.allow) {
				throw new AccessRuleValidationError(access);
			}
			...
			if (err instanceof AccessRuleValidationError) {
				...
				env.log.error(LOG_ACTION, "schema_validation_failed", { ...LOG_DETAILS, reason: (err as any).reason });
				res.status(422).send({ code: "schema_validation_failed", message: (err as any).message });
