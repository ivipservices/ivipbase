...
export abstract class WebSocketManager<SocketType> extends SimpleEventEmitter {
	...
	on(event: "signin", callback: WebSocketEventCallback<SocketType, { accessToken: string }>): Types.SimpleEventEmitterProperty;
	on(event: "signout", callback: WebSocketEventCallback<SocketType>): Types.SimpleEventEmitterProperty;

	on(event: "subscribe", callback: WebSocketEventCallback<SocketType, { req_id: string; path: string; event: string }>): Types.SimpleEventEmitterProperty;
	on(event: "unsubscribe", callback: WebSocketEventCallback<SocketType, { req_id: string; path: string; event?: string }>): Types.SimpleEventEmitterProperty;

	on(event: "query-unsubscribe", callback: WebSocketEventCallback<SocketType, { req_id: string; query_id: string }>): Types.SimpleEventEmitterProperty;
