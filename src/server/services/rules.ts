...
export type HasAccessResult = { allow: boolean; code?: RuleValidationFailCode; message?: string; rule?: PathRule; rulePath?: string; details?: Error } & (
	| { allow: true }
	| { allow: false; code: RuleValidationFailCode; message: string; rule?: PathRule; rulePath?: string; details?: Error }
);
...
export class PathBasedRules {
	...
	async isOperationAllowed(user: Pick<DbUserAccountDetails, "uid">, path: string, operation: AccessCheckOperation, data?: Record<string, any>): Promise<HasAccessResult> {
		...
		const getFullPath = (path: string, relativePath: string) => {
			...
			trailKeys.forEach((key) => {
				if (key === ".") {
					...
					targetPathInfo = targetPathInfo.parent as any;
				} else {
		...
		while (rule) {
			...
			for (const rule of checkRules) {
				...
				if (typeof rule === "string" || typeof rule === "function") {
					try {
						...
						return { allow: false, code: "exception", message: `${operation} operation denied to path "${path}" by set rule`, rule, rulePath, details: err as any };
					}
				}
			}
			if (isAllowed) {
			...
			let nextKey = pathKeys.shift() as string | number;
				...
				try {
					const result = await (async () => {
						...
						if (["cascade", "deny", "allow"].includes(result as string)) {
							return result as "cascade" | "deny" | "allow";
						}
