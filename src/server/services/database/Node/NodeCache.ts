...
export class NodeCacheEntry {
	...
	constructor(nodeInfo: NodeInfo) {
		...
		this.pathInfo = PathInfo.get(nodeInfo.path as any);
...
export class NodeCache {
	...
	private _cleanupTimeout: NodeJS.Timeout | null | undefined = null;
	...
	announce(path: string) {
		...
		if (!announcement) {
			...
			(announcement as any).promise = new Promise<NodeInfo>((resolve, reject) => {
				(announcement as any).resolve = resolve;
				(announcement as any).reject = reject;
			});

			this._announcements.set(path, announcement as any);
		}
	}
	...
	update(nodeInfo: NodeInfo) {
		...
		let entry = this._cache.get(nodeInfo.path as any);
		if (entry) {
			...
			this._cache.set(nodeInfo.path as any, entry);
		}
		const announcement = this._announcements.get(nodeInfo.path as any);
		if (announcement) {
			this._announcements.delete(nodeInfo.path as any);
	...
	delete(path: string) {
		const entry = this._cache.get(path);
		const pathInfo = PathInfo.get(path);
		DEBUG_MODE && console.error(`CACHE MARK_DELETED => ${(entry as any).nodeInfo}`);
	 ...
	 */
	find(path: string, checkAnnounced = false): NodeInfo | Promise<NodeInfo | null> | null {
