import { PathInfo, Lib, ID, DebugLogger } from "ivipbase-core";
...
export class NodeLocker {
	...
	_processLockQueue() {
		...
		const pending = this._locks
			.filter(
				(lock) => lock.state === LOCK_STATE.PENDING,
		...
		pending.forEach((lock) => {
			...
			lock.waitingFor = check.conflict || (null as any);
			if (check.allow) {
				this.lock(lock)
					.then(lock.resolve)
	 ...
	 */
	async lock(path: string, tid: string, forWriting?: boolean, comment?: string, options?: { withPriority?: boolean; noTimeout?: boolean }): Promise<NodeLock>;
	...
	async lock(
		...
		if (path instanceof NodeLock) {
		...
		} else if (typeof this._quit === "function" && !options.withPriority) {
			...
			// }
			lock = new NodeLock(this, path, tid as any, forWriting, options.withPriority === true);
			lock.comment = comment;
			this._locks.push(lock);
			const check = this._allowLock(path, tid as any, forWriting);
			lock.waitingFor = check.conflict || (null as any);
			proceed = check.allow;
		}
		if (proceed) {
				...
				if (options.noTimeout !== true) {
					...
					const timeoutHandler = () => {
						...
						if (timeoutCount <= 3) {
							// Warn first.
							this.debug.warn(
								`${lock.forWriting ? "write" : "read"} lock on path "/${lock.path}" by tid ${lock.tid} (${lock.comment}) is taking a long time to complete [${timeoutCount}]`,
							);
...
export class NodeLock {
	...
	expires!: number;
	comment = "";
	waitingFor: NodeLock | null | undefined = null;
	id: number = ++lastid;
	 ...
	 */
	constructor(private locker: NodeLocker, public path: string, public tid: string, public forWriting: boolean, public priority = false) {}
