	...
	return (function () {
	...
	})()(
		{
						...
						(function () {
							...
							class ConnectionSettings {
								constructor(settings) {
									...
									this.cache = {
										...
										priority:
											typeof ((_d = settings.cache) === null || _d === void 0 ? void 0 : _d.priority) === "string" && ["server", "cache"].includes(settings.cache.priority)
							...
							class AceBaseClient extends acebase_core_1.AceBaseBase {
								constructor(init) {
									...
									this.debug = new acebase_core_1.DebugLogger(settings.logLevel, `[${settings.dbname}]`.colorize(acebase_core_1.ColorStyle.blue));
			...
			2: [
				function (require, module, exports) {
					...
					class WebApi extends acebase_core_1.Api {
						...
						connect(retry = true) {
							...
							const transports = ((_a = this.settings.network) === null || _a === void 0 ? void 0 : _a.transports) instanceof Array ? this.settings.network.transports : ["websocket"];
							...
							return new Promise((resolve, reject) => {
								...
								socket.on("data-event", (data) => {
									...
									this._updateCursor(context.database_cursor);
									...
									targetSubs.forEach((subscr) => {
										...
										subscr.cursor = context.database_cursor;
						...
						async subscribe(path, event, callback, settings) {
							...
							const serverAlreadyNotifying =
								...
								(event === "mutated" &&
									Object.keys(this._subscriptions).some(
										(otherPath) =>
											acebase_core_1.PathInfo.get(otherPath).isAncestorOf(path) && this._subscriptions[otherPath].some((sub) => sub.event === event && sub.state === "active"),
									));
							...
							if (event === "mutated") {
								Object.keys(this._subscriptions)
									...
									.map((path) => _websocketRequest(this.socket, "unsubscribe", { path: path, event: "mutated" }, this.accessToken))
									.map((promise) => promise.catch((err) => console.error(err)));
							}
							const result = await _websocketRequest(this.socket, "subscribe", { path: path, event: event }, this.accessToken);
							subscr.activate();
						}
						async unsubscribe(path, event, callback) {
							...
							if (pathSubs.length === 0) {
								...
								if (this.isConnected) {
									promise = _websocketRequest(this.socket, "unsubscribe", { path: path, access_token: this.accessToken }, this.accessToken).catch((err) =>
										this.debug.error(`Failed to unsubscribe from event(s) on "${path}": ${err.message}`),
									);
								}
							} else if (this.isConnected && !pathSubs.some((subscr) => subscr.event === event)) {
								promise = _websocketRequest(this.socket, "unsubscribe", { path: path, event: event, access_token: this.accessToken }, this.accessToken).catch((err) =>
						...
						transaction(path, callback, options = { context: {} }) {
							...
							options.context.acebase_mutation = { client_id: this._id, id: id, op: "transaction", path: path, flow: "server" };
							...
							return new Promise(async (resolve, reject) => {
								...
								const handleSuccess = async (context) => {
									...
									resolve({ cursor: context === null || context === void 0 ? void 0 : context.database_cursor });
								};
								if (this.isConnected && ((_a = this.settings.network) === null || _a === void 0 ? void 0 : _a.realtime)) {
									...
									const startedCallback = async (data) => {
										if (data.id === id) {
											...
											socket.emit("transaction", { action: "finish", id: id, path: path, value: acebase_core_1.Transport.serialize(newValue), access_token: this.accessToken });
											if (this.hasCache) {
									...
									socket.emit("transaction", { action: "start", id: id, path: path, access_token: this.accessToken, context: options.context });
								} else {
						...
						async _request(options) {
							...
							if (this.isConnected || options.ignoreConnectionState === true) {
								...
								if (result.context && result.context.database_cursor) {
									this._updateCursor(result.context.database_cursor);
								}
						...
						async signIn(username, password) {
							...
							const result = await this._request({
								...
								data: { method: "account", username: username, password: password, client_id: this.socket && this.socket.id },
							});
						...
						async forgotPassword(email) {
							...
							const result = await this._request({ method: "POST", url: `${this.url}/auth/${this.dbname}/forgot_password`, data: { email: email } });
							return result;
						}
						...
						async deleteAccount(uid, signOut = true) {
							...
							const result = await this._request({ method: "POST", url: `${this.url}/auth/${this.dbname}/delete`, data: { uid: uid } });
							if (signOut) {
								this.socket && this.socket.emit("signout", this.accessToken);
								this.accessToken = null;
								this.eventCallback("signout");
							...
							try {
								...
								if (this.hasCache) {
									...
									try {
										...
										cacheApi
											.set(`${this.dbname}/stats/last_sync_error`, { date: new Date(), code: err.code || "unknown", message: err.message, stack: err.stack })
								...
								const info = { local: totalPendingChanges, remote: totalRemoteChanges, method: usedSyncMethod, cursor: cursor };
						...
						async getMutations(filter) {
							...
							const query = Object.keys(filter)
								...
								.filter((p) => p != null)
								.join("&");
							const { data: data, context: context } = await this._request({ url: `${this.url}/sync/mutations/${this.dbname}?${query}`, includeContext: true });
							const mutations = acebase_core_1.Transport.deserialize2(data);
							return { used_cursor: (_a = filter.cursor) !== null && _a !== void 0 ? _a : null, new_cursor: context.database_cursor, mutations: mutations };
						}
						async getChanges(filter) {
							...
							const query = Object.keys(filter)
								...
								.filter((p) => p != null)
								.join("&");
							const { data: data, context: context } = await this._request({ url: `${this.url}/sync/changes/${this.dbname}?${query}`, includeContext: true });
							const changes = acebase_core_1.Transport.deserialize2(data);
							return { used_cursor: (_a = filter.cursor) !== null && _a !== void 0 ? _a : null, new_cursor: context.database_cursor, changes: changes };
						}
						async _addCacheSetMutation(path, value, context) {
							...
							return (_b = this._cache) === null || _b === void 0
								? void 0
								: _b.db.api.set(`${this.dbname}/pending/${acebase_core_1.ID.generate()}`, { type: value !== null ? "set" : "remove", path: path, data: value, context: context });
						}
						set(path, value, options = { allow_cache: true, context: {} }) {
							...
							const tryServerPromise = !useServer ? null : serverPromise.then(() => ({ success: true, error: null })).catch((err) => ({ success: false, error: err }));
							Promise.all([tryCachePromise, tryServerPromise]).then(([cacheResult, serverResult]) => {
								...
								const networkError =
									...
									((_a = serverResult === null || serverResult === void 0 ? void 0 : serverResult.error) === null || _a === void 0 ? void 0 : _a.isNetworkError) === true;
						...
						update(path, updates, options = { allow_cache: true, context: {} }) {
							...
							const tryServerPromise = !useServer
								...
								: serverPromise.then(() => ({ executed: true, success: true, error: null })).catch((err) => ({ executed: true, success: false, error: err }));
							Promise.all([tryCachePromise, tryServerPromise]).then(([cacheResult, serverResult]) => {
						...
						async get(path, options = { cache_mode: "allow" }) {
							...
							const getServerValue = async () => {
								...
								const cursor = context && context.database_cursor;
								const value = acebase_core_1.Transport.deserialize(result.data);
								if (this._cache) {
									if (!filtered) {
										const cachePath = acebase_core_1.PathInfo.getChildPath(`${this.dbname}/cache`, path);
										this._cache.db.api.set(cachePath, value, { context: { acebase_operation: "update_cache", acebase_server_context: context } }).catch((err) => {
							...
							if (useCache && typeof options.cache_cursor === "string") {
								...
								const { value: value, context: context } = await getCacheValue(false);
								if (syncResult) {
									context.database_cursor = syncResult.new_cursor;
									context.acebase_origin = "hybrid";
								} else {
									context.database_cursor = options.cache_cursor;
									context.acebase_origin = "cache";
								}
								return { value: value, context: context, cursor: context.database_cursor };
							}
							if (!useCache) {
						...
						async updateCache(path = "", cursor) {
							...
							if (loadValue) {
								...
								return { path: path, used_cursor: cursor, new_cursor: context.database_cursor, loaded_value: true, changes: [] };
							}
						...
						async query(path, query, options = { snapshots: false, cache_mode: "allow", monitor: { add: false, change: false, remove: false } }) {
							...
							try {
								const { data: data, context: context } = await this._request({ method: "POST", url: `${this.url}/query/${this.dbname}/${path}`, data: reqData, includeContext: true });
				...
				{ "./base64": 4, "./errors": 6, "./promise-timeout": 9, "./request": 10, "./request/error": 11, "acebase-core": 25, "socket.io-client": 41 },
			],
			3: [
				function (require, module, exports) {
					"use strict";
			...
			14: [
				...
				{ "./data-reference": 21, "./debug": 23, "./optional-observable": 27, "./simple-colors": 34, "./simple-event-emitter": 35, "./type-mappings": 38 },
			],
			15: [
				function (require, module, exports) {
					"use strict";
			...
			17: [
				function (require, module, exports) {
					...
					const env = typeof window === "object" ? window : self,
						...
						mimeTypesLength = (_b = (_a = navigator.mimeTypes) === null || _a === void 0 ? void 0 : _a.length) !== null && _b !== void 0 ? _b : 0,
						clientId = (0, pad_1.default)((mimeTypesLength + navigator.userAgent.length).toString(36) + globalCount.toString(36), 4);
					...
					class LiveDataProxy {
						static async create(ref, options) {
							...
							const subscription = ref.on("mutations", { syncFallback: syncFallback }).subscribe(async (snap) => {
								...
								if (proceed) {
									clientEventEmitter.emit("cursor", context.database_cursor);
							...
							const flagOverwritten = (target) => {
								if (!mutationQueue.find((m) => RelativeNodeTarget.areEqual(m.target, target))) {
									mutationQueue.push({ target: target, previous: (0, utils_1.cloneObject)(getTargetValue(cache, target)) });
								}
								scheduleSync();
							};
							...
							const addOnChangeHandler = (target, callback) => {
								...
								const mutationsHandler = async (details) => {
									...
									const mutations = snap.val(false).filter((mutation) => mutation.target.slice(0, target.length).every((key, i) => target[i] === key));
									if (mutations.length === 0) {
										return;
									}
							...
							const handleFlag = (flag, target, args) => {
								if (flag === "write") {
									...
									const hasConflictingTransaction = transactions.some(
										(t) => RelativeNodeTarget.areEqual(target, t.target) || RelativeNodeTarget.isAncestor(target, t.target) || RelativeNodeTarget.isDescendant(target, t.target),
									);
					...
					function createProxy(context) {
						...
						const handler = {
							get(target, prop, receiver) {
								...
								const unproxyValue = (value) => (value !== null && typeof value === "object" && value[isProxy] ? value.getTarget() : value);
								if (
									...
									if (isArray) {
										...
										if (["reduce", "reduceRight"].includes(prop)) {
											return function reduce(callback, initialValue) {
												return target[prop]((prev, value, i) => callback(prev, proxifyChildValue(i), i, proxy), initialValue);
											};
										}
			...
			21: [
				function (require, module, exports) {
					...
					class DataRetrievalOptions {
						constructor(options) {
							...
							this.cache_mode =
								typeof options.cache_mode === "string" ? options.cache_mode : typeof options.allow_cache === "boolean" ? (options.allow_cache ? "allow" : "bypass") : "allow";
						...
						async update(updates, onComplete) {
							try {
								...
								if (typeof updates !== "object" || updates instanceof Array || updates instanceof ArrayBuffer || updates instanceof Date) {
									...
									const { cursor: cursor } = await this.db.api.update(this.path, updates, { context: this[_private].context });
									this.cursor = cursor;
								}
								if (typeof onComplete === "function") {
									try {
							...
							const cb = {
								...
								ourCallback: (err, path, newValue, oldValue, eventContext) => {
									...
									if (event.startsWith("notify_")) {
										...
										const values = { previous: this.db.types.deserialize(path, oldValue), current: this.db.types.deserialize(path, newValue) };
				...
				{ "./data-proxy": 20, "./data-snapshot": 22, "./id": 24, "./optional-observable": 27, "./path-info": 29, "./subscription": 36 },
			],
			22: [
				function (require, module, exports) {
					"use strict";
				...
				function (require, module, exports) {
							...
							if (name.length === 0) {
								if (definition[pos] === "*") {
									...
									while (true) {
										...
										type.children.push({ name: prop.name, optional: prop.optional, wildcard: prop.wildcard, types: types });
										consumeSpaces();
										if (definition[pos] === "}") {
											break;
										}
					...
					function checkObject(path, properties, obj, partial) {
						const invalidProperties = properties.find((prop) => prop.name === "*" || prop.name[0] === "$")
							? []
							: Object.keys(obj).filter((key) => ![null, undefined].includes(obj[key]) && !properties.find((prop) => prop.name === key));
						if (invalidProperties.length > 0) {
							return {
								ok: false,
								reason: `Object at path "${path}" cannot have propert${invalidProperties.length === 1 ? "y" : "ies"} ${invalidProperties.map((p) => `"${p}"`).join(", ")}`,
							};
						}
			...
			34: [
				function (require, module, exports) {
					...
					const ColorCode = { black: 30, red: 31, green: 32, yellow: 33, blue: 34, magenta: 35, cyan: 36, white: 37, grey: 90, brightRed: 91 };
					const BgColorCode = { bgBlack: 40, bgRed: 41, bgGreen: 42, bgYellow: 43, bgBlue: 44, bgMagenta: 45, bgCyan: 46, bgWhite: 47, bgGrey: 100, bgBrightRed: 101 };
					const ResetCode = { all: 0, color: 39, background: 49, bold: 22, dim: 22, italic: 23, underline: 24, inverse: 27, hidden: 28, strikethrough: 29 };
					var ColorStyle;
					(function (ColorStyle) {
			...
			38: [
				function (require, module, exports) {
					...
					function process(db, mappings, path, obj, action) {
						...
						const changes = [];
						m.sort((a, b) => (path_info_1.PathInfo.getPathKeys(a.path).length > path_info_1.PathInfo.getPathKeys(b.path).length ? -1 : 1));
						m.forEach((mapping) => {
					...
					(function (global, Buffer) {
						(function () {
							...
							function cloneObject(original, stack) {
								...
								const checkAndFixTypedArray = (obj) => {
									if (
										...
										["Buffer", "Uint8Array", "Int8Array", "Uint16Array", "Int16Array", "Uint32Array", "Int32Array", "BigUint64Array", "BigInt64Array"].includes(
								...
								const cloneValue = (val) => {
									...
									if (val === null || val instanceof Date || val instanceof ArrayBuffer || val instanceof path_reference_1.PathReference || val instanceof RegExp) {
										return val;
									} else if (typeof val === "object") {
										stack.push(val);
							...
							const isTypedArray = (val) =>
								typeof val === "object" &&
								["ArrayBuffer", "Buffer", "Uint8Array", "Uint16Array", "Uint32Array", "Int8Array", "Int16Array", "Int32Array"].includes(val.constructor.name);
							...
							function compareValues(oldVal, newVal, sortedResults = false) {
								...
								if (oldVal === newVal) {
									...
									return oldVal instanceof path_reference_1.PathReference && newVal instanceof path_reference_1.PathReference && oldVal.path === newVal.path
										? "identical"
										: "changed";
							...
							!(function (t, e) {
								...
								return (function (t) {
									...
									function (t, e) {
										function r(t) {
											for (var e, r, n = [], o = 0, i = t.length; o < i; ) {
												(e = t.charCodeAt(o++)),
													e >= 55296 && e <= 56319 && o < i
														? ((r = t.charCodeAt(o++)), 56320 == (64512 & r) ? n.push(((1023 & e) << 10) + (1023 & r) + 65536) : (n.push(e), o--))
														: n.push(e);
											}
											return n;
										}
										...
										function s(t, e) {
											...
											return (
												0 == (4294965248 & t)
													...
													: 0 == (4292870144 & t) && ((r = d(((t >> 18) & 7) | 240)), (r += i(t, 12)), (r += i(t, 6))),
												(r += d((63 & t) | 128))
											);
										}
										function a(t, e) {
										...
										function p(t) {
											...
											if (((e = 255 & u[l]), l++, 0 == (128 & e))) {
												return e;
											}
											if (192 == (224 & e)) {
												if (((r = c()), (s = ((31 & e) << 6) | r), s >= 128)) {
													return s;
												}
												throw Error("Invalid continuation byte");
											}
											...
											if (240 == (248 & e) && ((r = c()), (n = c()), (i = c()), (s = ((7 & e) << 18) | (r << 12) | (n << 6) | i), s >= 65536 && s <= 1114111)) {
												return s;
											}
									...
									function (t, e, r) {
										...
										if (
											("undefined" != typeof WebSocket ? (o = WebSocket) : "undefined" != typeof self && (o = self.WebSocket || self.MozWebSocket), "undefined" == typeof window)
										) {
											try {
												i = r(32);
											} catch (t) {}
									...
									function (t, e, r) {
										...
										s(n.prototype),
											...
											(n.prototype.open = n.prototype.connect =
												function () {
													return this.connected
														? this
														: (this.subEvents(), this.io.reconnecting || this.io.open(), "open" === this.io.readyState && this.onopen(), this.emit("connecting"), this);
												}),
											...
											(n.prototype.emit = function (t) {
												...
												var e = a(arguments),
													r = { type: (void 0 !== this.flags.binary ? this.flags.binary : u(e)) ? i.BINARY_EVENT : i.EVENT, data: e };
												return (
													(r.options = {}),
													(r.options.compress = !this.flags || !1 !== this.flags.compress),
													"function" == typeof e[e.length - 1] && ((this.acks[this.ids] = e.pop()), (r.id = this.ids++)),
