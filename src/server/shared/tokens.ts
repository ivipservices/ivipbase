...
export const createPublicAccessToken = (database: string, uid: string, ip: string, dbToken: string, password: string) => {
	const obj = {
		t: dbToken,
		d: database,
		c: Date.now(),
...
export type PublicAccessToken = { access_token: string; database: string; uid: string; created: number; ip: string };
