        ...
        try {
            ...
            if (impersonatedAccess.uid) {
                ...
                for (const childInfo of list ?? []) {
                    childInfo.access = {
                        read: (await env.rules.isOperationAllowed(impersonatedUser, acebase_core_1.PathInfo.getChildPath(path, childInfo.key), 'get')).allow,
                        write: (await env.rules.isOperationAllowed(impersonatedUser, acebase_core_1.PathInfo.getChildPath(path, childInfo.key), 'update')).allow // Use pre-flight 'update' check to mimic legacy 'write' check
                    };
                }
            }
            res.send(result);
