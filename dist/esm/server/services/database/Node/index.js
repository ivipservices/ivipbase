...
export * from "./NodeCache.js";
export * from "./NodeChanges.js";
export { default as NodeInfo } from "./NodeInfo.js";
export * from "./NodeLock.js";
...
export default class Node extends SimpleEventEmitter {
    ...
    async synchronize(path, allHeirs = false) {
        ...
        if (typeof this.settings.dataSynchronization === "function") {
            const nodes = (await this.settings.dataSynchronization(fullRegex, "get")) ?? [];
            this.push(nodes);
        }
    }
    /**
    ...
    getInfoBy(path, options = {}) {
        ...
        info.value = value ? prepareValue(value) : [VALUE_TYPES.OBJECT, VALUE_TYPES.ARRAY].includes(info.type) ? (info.type === VALUE_TYPES.ARRAY ? [] : {}) : null;
        if (options.include_child_count) {
            info.childCount = value ? Object.keys(value ?? {}).length : 0;
            if (containsChild && [VALUE_TYPES.OBJECT, VALUE_TYPES.ARRAY].includes(info.valueType) && info.address) {
    ...
    getNode(path, onlyChildren = false, includeChildrenDedicated = true) {
        const pathRoot = new PathInfo(path);
