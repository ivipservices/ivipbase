...
const StorageNode_1 = require("../lib/StorageNode.js");
    ...
    async setNode(path, value, options = { suppress_events: false, context: null }) {
        ...
        try {
            if (path === '') {
                ...
                await this._writeNodeWithTracking('', value, {
                    merge: false,
                    transaction,
                ...
                if (info.address && info.address.path === path && value !== null && !this.valueFitsInline(value)) {
                    // Overwrite node
                    await this._writeNodeWithTracking(path, value, {
                        merge: false,
                        transaction,
                        suppress_events: options.suppress_events,
                        context: options.context,
    ...
    async updateNode(path, updates, options = { suppress_events: false, context: null }) {
        ...
        try {
            ...
            if (nodeInfo.exists && nodeInfo.address && nodeInfo.address.path === path) {
                ...
                // Update it
                await this._writeNodeWithTracking(path, updates, {
                    transaction,
                    merge: true,
                    suppress_events: options.suppress_events,
                    context: options.context,
