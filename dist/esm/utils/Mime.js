...
export const define = (typeMap, force) => {
    for (let type in typeMap) {
        ...
        if (force || !extensions.has(type)) {
            const ext = exts[0];
            extensions.set(type, ext[0] !== "*" ? ext : ext.substr(1));
        }
    }
};
define(standard);
...
export const getType = (path) => {
    ...
    return ((hasDot || !hasPath) && types.get(ext)) || "application/octet-binary";
};
export const getExtension = (type) => {
    // type = /^\s*([^;\s]*)/.test(type) && RegExp.$1;
    const t = /^\s*([^;\s]*)/.test(type);
    return (t && extensions.get(type.toLowerCase())) || null;
};
