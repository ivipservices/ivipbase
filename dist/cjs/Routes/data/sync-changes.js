...
const addRoute = (env) => {
    env.router.get(`/sync/changes/${env.db.name}`, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
        var _a;
        ...
        if (!((_a = env.config.transactions) === null || _a === void 0 ? void 0 : _a.log)) {
            return (0, Errors_1.sendBadRequestError)(res, { code: "no_transaction_logging", message: "Transaction logging not enabled" });
        }
        try {
            const data = req.query;
            const targets = typeof data.path === "string" ? [{ path: data.path, events: ["value"] }] : typeof data.for === "string" ? JSON.parse(data.for) : null;
            if (targets === null) {
                return (0, Errors_1.sendBadRequestError)(res, { code: "invalid_request", message: "Invalid changes request" });
            }
            if (targets.length === 0) {
            ...
            for (let i = 0; i < targets.length; i++) {
                ...
                if (target.events.every((event) => /^(?:notify_)?child_/.test(event))) {
                    //if (!target.events.some(event => ['value','notify_value','mutations','mutated'].includes(event))) {
            ...
            if (targets.length === 0) {
                return (0, Errors_1.sendUnauthorizedError)(res, "not_authorized", "User is not authorized to access this data");
            }
            const { cursor } = data;
            let timestamp;
            if (typeof data.timestamp !== "undefined") {
                ...
                if (isNaN(timestamp)) {
                    return (0, Errors_1.sendBadRequestError)(res, { code: "wrong_timestamp", message: "Timestamp is not a number" });
                }
            }
...
//# sourceMappingURL=sync-changes.js.map