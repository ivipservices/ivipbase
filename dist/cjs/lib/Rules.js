...
class PathBasedRules {
    ...
    isOperationAllowed(user, path, operation, data) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const getFullPath = (path, relativePath) => {
                ...
                let targetPathInfo = PathInfo_1.PathInfo.get(path);
                const trailKeys = PathInfo_1.PathInfo.getPathKeys(relativePath);
            ...
            while (rule) {
                ...
                const rulePath = PathInfo_1.PathInfo.get(rulePathKeys).path;
                for (const rule of checkRules) {
                    ...
                    if (typeof rule === "string" || typeof rule === "function") {
                        try {
                            ...
                            const result = typeof rule === "function" ? yield rule(ruleEnv) : yield (0, Sandbox_1.executeSandboxed)(rule, ruleEnv);
                ...
                currentPath = PathInfo_1.PathInfo.get(currentPath).childPath(nextKey);
            ...
            if (isAllowed && ["set", "update"].includes(operation) && !isPreFlight) {
                ...
                for (const check of checkRules) {
                    ...
                    const validatePath = PathInfo_1.PathInfo.get(path).child(check.target).path;
    ...
    add(rulePaths, ruleTypes, callback) {
        ...
        for (const path of paths) {
            const keys = PathInfo_1.PathInfo.getPathKeys(path);
