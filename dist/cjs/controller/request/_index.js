        ...
        const req = client.request(request, (res) => {
            ...
            res.on("end", () => {
                ...
                if (res.statusCode === 200) {
                    const contextHeader = res.headers["database-context"]; // lowercase header names only
                    let context;
                    ...
                    return reject(new error_1.RequestError(request, response, code, message));
                }
            });
        });
        req.on("error", (err) => {
            reject(new error_1.RequestError(request, null, err.code || err.name, err.message));
        });
        if (typeof options.dataRequestCallback === "function") {
