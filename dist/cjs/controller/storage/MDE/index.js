...
class MDE extends ivipbase_core_1.SimpleEventEmitter {
    ...
    async getNodesBy(database, path, onlyChildren = false, allHeirs = false, includeAncestor = false, simplifyValues = false) {
        ...
        const mainNode = result.find(({ path: p }) => ivipbase_core_1.PathInfo.get(p).equals(path));
        const parentNode = result.find(({ path: p }) => ivipbase_core_1.PathInfo.get(p).isParentOf(path));
        const isMainChild = !mainNode && parentNode;
        return result.filter(({ path: p, content }) => {
            const path = ivipbase_core_1.PathInfo.get(p);
            let includes = true;
            if (isMainChild) {
    ...
    async getInfoBy(database, path, options = {}) {
        ...
        const egualsNode = nodes.find(({ path: p }) => ivipbase_core_1.PathInfo.get(p).equals(pathInfo.path));
        const parentNode = nodes.find(({ path: p }) => ivipbase_core_1.PathInfo.get(p).isParentOf(pathInfo.path));
        const mainNode = egualsNode !== null && egualsNode !== void 0 ? egualsNode : parentNode;
