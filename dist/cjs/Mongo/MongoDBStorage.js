...
exports.CustomStorage = exports.CustomStorageNodeInfo = exports.CustomStorageNodeAddress = void 0;
const acebase_core_1 = require("acebase-core");
const { compareValues } = acebase_core_1.Utils;
const node_info_1 = require("acebase/dist/types/node-info");
const storage_1 = require("acebase/dist/types/storage");
const node_errors_1 = require("acebase/dist/types/node-errors");
const node_address_1 = require("acebase/dist/types/node-address");
const Colorize_1 = require("../lib/Colorize");
const PathInfo_1 = require("../lib/PathInfo");
const assert_1 = require("../lib/assert");
const StorageNode_1 = require("../lib/StorageNode");
...
class CustomStorage extends storage_1.Storage {
    ...
    _init() {
        return __awaiter(this, void 0, void 0, function* () {
            this.debug.log(`Database "${this.name}" details:`.colorize(Colorize_1.ColorStyle.dim));
            this.debug.log(`- Type: CustomStorage`.colorize(Colorize_1.ColorStyle.dim));
            this.debug.log(`- Path: ${this.settings.path}`.colorize(Colorize_1.ColorStyle.dim));
            this.debug.log(`- Max inline value size: ${this.settings.maxInlineValueSize}`.colorize(Colorize_1.ColorStyle.dim));
            this.debug.log(`- Autoremove undefined props: ${this.settings.removeVoidProperties}`.colorize(Colorize_1.ColorStyle.dim));
            // Create root node if it's not there yet
            yield this._customImplementation.ready();
            const transaction = yield this._customImplementation.getTransaction({ path: '', write: true });
            const info = yield this.getNodeInfo('', { transaction });
            if (!info.exists) {
                yield this._writeNode('', {}, { transaction });
            }
            yield transaction.commit();
            if (this.indexes.supported) {
                yield this.indexes.load();
            }
            this.emit('ready');
        });
    }
    ...
    _storeNode(path, node, options) {
        ...
        const getTypedChildValue = (val) => {
            if (val === null) {
                ...
                return { type: StorageNode_1.VALUE_TYPES.DATETIME, value: val.getTime() };
            }
            else if (val instanceof acebase_core_1.PathReference) {
                return { type: StorageNode_1.VALUE_TYPES.REFERENCE, value: val.path };
            }
            else if (val instanceof ArrayBuffer) {
                return { type: StorageNode_1.VALUE_TYPES.BINARY, value: acebase_core_1.ascii85.encode(val) };
            }
        ...
        if (node.type === StorageNode_1.VALUE_TYPES.ARRAY && node.value instanceof Array) {
        ...
        if (node.type === StorageNode_1.VALUE_TYPES.BINARY && typeof node.value !== 'string') {
            console.warn(`Unprocessed binary value. ${unprocessed}`);
            node.value = acebase_core_1.ascii85.encode(node.value);
        }
        if (node.type === StorageNode_1.VALUE_TYPES.REFERENCE && node.value instanceof acebase_core_1.PathReference) {
            console.warn(`Unprocessed path reference. ${unprocessed}`);
            node.value = node.value.path;
        }
        if ([StorageNode_1.VALUE_TYPES.OBJECT, StorageNode_1.VALUE_TYPES.ARRAY].includes(node.type)) {
    ...
    _processReadNodeValue(node) {
        const getTypedChildValue = (val) => {
            ...
            if (val.type === StorageNode_1.VALUE_TYPES.BINARY) {
                // binary stored in a parent record as a string
                return acebase_core_1.ascii85.decode(val.value);
            }
            else if (val.type === StorageNode_1.VALUE_TYPES.DATETIME) {
                // Date value stored as number
                return new Date(val.value);
            }
            else if (val.type === StorageNode_1.VALUE_TYPES.REFERENCE) {
        ...
        switch (node.type) {
            case StorageNode_1.VALUE_TYPES.ARRAY:
            case StorageNode_1.VALUE_TYPES.OBJECT: {
            ...
            case StorageNode_1.VALUE_TYPES.BINARY: {
                node.value = acebase_core_1.ascii85.decode(node.value);
                break;
            }
            case StorageNode_1.VALUE_TYPES.REFERENCE: {
                node.value = new acebase_core_1.PathReference(node.value);
                break;
            }
            case StorageNode_1.VALUE_TYPES.STRING: {
                // No action needed
    ...
    _getTypeFromStoredValue(val) {
        ...
        if (typeof val === 'string') {
            type = StorageNode_1.VALUE_TYPES.STRING;
        }
        else if (typeof val === 'number') {
            type = StorageNode_1.VALUE_TYPES.NUMBER;
        }
        else if (typeof val === 'boolean') {
            type = StorageNode_1.VALUE_TYPES.BOOLEAN;
        }
        else if (val instanceof Array) {
            type = StorageNode_1.VALUE_TYPES.ARRAY;
        }
        else if (typeof val === 'object') {
            if ('type' in val) {
                ...
                if (type === StorageNode_1.VALUE_TYPES.DATETIME) {
                    val = new Date(val);
                }
                else if (type === StorageNode_1.VALUE_TYPES.REFERENCE) {
                    val = new acebase_core_1.PathReference(val);
                }
            }
            else {
                type = StorageNode_1.VALUE_TYPES.OBJECT;
            }
        }
        else {
    ...
    _writeNode(path, value, options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const currentRow = options.currentValue === null
                ...
                : yield this._readNode(path, { transaction });
            if (options.merge && currentRow) {
                if (currentRow.type === StorageNode_1.VALUE_TYPES.ARRAY && !(value instanceof Array) && typeof value === 'object' && Object.keys(value).some((key) => isNaN(parseInt(key)))) {
                    throw new Error(`Cannot merge existing array of path "${path}" with an object`);
                }
                if (value instanceof Array && currentRow.type !== StorageNode_1.VALUE_TYPES.ARRAY) {
            ...
            const mainNode = {
                type: currentRow && currentRow.type === StorageNode_1.VALUE_TYPES.ARRAY ? StorageNode_1.VALUE_TYPES.ARRAY : StorageNode_1.VALUE_TYPES.OBJECT,
                value: {},
            };
            const childNodeValues = {};
            if (value instanceof Array) {
                mainNode.type = StorageNode_1.VALUE_TYPES.ARRAY;
                // Convert array to object with numeric properties
                const obj = {};
                ...
                mainNode.type = StorageNode_1.VALUE_TYPES.REFERENCE;
                mainNode.value = value.path;
            }
            else if (value instanceof ArrayBuffer) {
                mainNode.type = StorageNode_1.VALUE_TYPES.BINARY;
                mainNode.value = acebase_core_1.ascii85.encode(value);
            }
            else if (typeof value === 'string') {
                mainNode.type = StorageNode_1.VALUE_TYPES.STRING;
                mainNode.value = value;
            }
            const currentIsObjectOrArray = currentRow ? [StorageNode_1.VALUE_TYPES.OBJECT, StorageNode_1.VALUE_TYPES.ARRAY].includes(currentRow.type) : false;
            const newIsObjectOrArray = [StorageNode_1.VALUE_TYPES.OBJECT, StorageNode_1.VALUE_TYPES.ARRAY].includes(mainNode.type);
            const children = {
                current: [],
                new: [],
            };
            ...
            const isArray = mainNode.type === StorageNode_1.VALUE_TYPES.ARRAY;
            if (currentRow) {
                // update
                this.debug.log(`Node "/${path}" is being ${options.merge ? 'updated' : 'overwritten'}`.colorize(Colorize_1.ColorStyle.cyan));
                ...
                if (currentIsObjectOrArray || newIsObjectOrArray) {
                    ...
                    const addChildPath = (childPath) => {
                        ...
                        const key = PathInfo_1.PathInfo.get(childPath).key;
                        keys.push(key.toString()); // .toString to make sure all keys are compared as strings
                        return true; // Keep streaming
                    };
                    yield transaction.childrenOf(path, { metadata: false, value: false }, includeChildCheck, addChildPath);
                    ...
                    const writePromises = Object.keys(childNodeValues).map((key) => {
                        ...
                        const currentChildValue = typeof options.currentValue === 'undefined' // Fixing issue #20
                            ...
                            : options.currentValue !== null && typeof options.currentValue === 'object' && keyOrIndex in options.currentValue
                                ...
                                : null;
                        return this._writeNode(childPath, childValue, {
                            transaction,
                            revision,
                            merge: false,
                ...
                this.debug.log(`Node "/${path}" is being created`.colorize(Colorize_1.ColorStyle.cyan));
                if (isArray) {
                ...
                const promises = Object.keys(childNodeValues).map((key) => {
                    const keyOrIndex = isArray ? parseInt(key) : key;
                    const childPath = PathInfo_1.PathInfo.getChildPath(path, keyOrIndex);
                    const childValue = childNodeValues[keyOrIndex];
     ...
     */
    _deleteNode(path, options) {
        return __awaiter(this, void 0, void 0, function* () {
            const pathInfo = PathInfo_1.PathInfo.get(path);
            this.debug.log(`Node "/${path}" is being deleted`.colorize(Colorize_1.ColorStyle.cyan));
            ...
            const transaction = options.transaction;
            yield transaction.descendantsOf(path, { metadata: false, value: false }, includeDescendantCheck, addDescendant);
            this.debug.log(`Nodes ${deletePaths.map((p) => `"/${p}"`).join(',')} are being deleted`.colorize(Colorize_1.ColorStyle.cyan));
    ...
    getChildren(path, options = {}) {
        ...
        const start = () => __awaiter(this, void 0, void 0, function* () {
            const transaction = options.transaction || (yield this._customImplementation.getTransaction({ path, write: false }));
            try {
                let canceled = false;
                yield (() => __awaiter(this, void 0, void 0, function* () {
                    const node = yield this._readNode(path, { transaction });
                    if (!node) {
                    ...
                    const isArray = node.type === StorageNode_1.VALUE_TYPES.ARRAY;
                    ...
                    yield transaction.childrenOf(path, { metadata: true, value: false }, includeChildCheck, addChildNode);
                }))();
                ...
                if (!options.transaction) {
                    ...
                    yield transaction.rollback(err);
                }
                throw err;
            }
        }); // start()
    ...
    getNode(path, options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const transaction = options.transaction || (yield this._customImplementation.getTransaction({ path, write: false }));
            try {
                const node = yield (() => __awaiter(this, void 0, void 0, function* () {
                    ...
                    const pathInfo = PathInfo_1.PathInfo.get(path);
                    const targetNode = yield this._readNode(path, { transaction });
                    if (!targetNode) {
                        // Lookup parent node
                        if (path === '') {
                            return { value: null };
                        } // path is root. There is no parent.
                        const lockPath = yield transaction.moveToParentPath(pathInfo.parentPath);
                        ...
                        const parentNode = yield this._readNode(pathInfo.parentPath, { transaction });
                        if (parentNode && [StorageNode_1.VALUE_TYPES.OBJECT, StorageNode_1.VALUE_TYPES.ARRAY].includes(parentNode.type) && pathInfo.key in parentNode.value) {
                    ...
                    const isArray = targetNode.type === StorageNode_1.VALUE_TYPES.ARRAY;
                    /**
                    ...
                    const convertFilterArray = (arr) => {
                        ...
                        return arr.map((path) => PathInfo_1.PathInfo.get(isArray && isNumber(path) ? `[${path}]` : path));
                    };
                    ...
                    const applyFiltersOnInlineData = (descPath, node) => {
                        if ([StorageNode_1.VALUE_TYPES.OBJECT, StorageNode_1.VALUE_TYPES.ARRAY].includes(node.type) && includeFilter.length > 0) {
                            const trailKeys = PathInfo_1.PathInfo.getPathKeys(descPath).slice(pathInfo.keys.length);
                            const checkPathInfo = new PathInfo_1.PathInfo(trailKeys);
                            const remove = [];
                    ...
                    const includeDescendantCheck = (descPath, metadata) => {
                        ...
                        const descPathKeys = PathInfo_1.PathInfo.getPathKeys(descPath);
                        const trailKeys = descPathKeys.slice(pathInfo.keys.length);
                        const checkPathInfo = new PathInfo_1.PathInfo(trailKeys);
                        let include = (includeFilter.length > 0 ? includeFilter.some((info) => checkPathInfo.isOnTrailOf(info)) : true) &&
                            (excludeFilter.length > 0 ? !excludeFilter.some((info) => info.equals(checkPathInfo) || info.isAncestorOf(checkPathInfo)) : true);
                        ...
                        if (include &&
                            ...
                            ((pathInfo.isParentOf(descPath) && [StorageNode_1.VALUE_TYPES.OBJECT, StorageNode_1.VALUE_TYPES.ARRAY].includes(metadata ? metadata.type : -1)) ||
                                PathInfo_1.PathInfo.getPathKeys(descPath).length > pathInfo.pathKeys.length + 1)) {
                            include = false;
                        }
                        return include;
                    };
                    ...
                    const addDescendant = (descPath, node) => {
                        ...
                        if (options.child_objects === false && [StorageNode_1.VALUE_TYPES.OBJECT, StorageNode_1.VALUE_TYPES.ARRAY].includes(node.type)) {
                    ...
                    yield transaction.descendantsOf(path, { metadata: true, value: true }, includeDescendantCheck, addDescendant);
                    this.debug.log(`Read node "/${path}" and ${filtered ? '(filtered) ' : ''}descendants from ${descRows.length + 1} records`.colorize(Colorize_1.ColorStyle.magenta));
                    ...
                    if (targetNode.type === StorageNode_1.VALUE_TYPES.ARRAY) {
                        result.value = objectToArray(result.value);
                    }
                    if (targetNode.type === StorageNode_1.VALUE_TYPES.OBJECT || targetNode.type === StorageNode_1.VALUE_TYPES.ARRAY) {
                        // target node is an object or array
                        // merge with other found (child) nodes
                        const targetPathKeys = PathInfo_1.PathInfo.getPathKeys(path);
                        const value = targetNode.value;
                        for (let i = 0; i < descRows.length; i++) {
                            const otherNode = descRows[i];
                            const pathKeys = PathInfo_1.PathInfo.getPathKeys(otherNode.path);
                            ...
                            for (let j = 0; j < trailKeys.length; j++) {
                                ...
                                const nodeType = isLast ? otherNode.type : typeof trailKeys[j + 1] === 'number' ? StorageNode_1.VALUE_TYPES.ARRAY : StorageNode_1.VALUE_TYPES.OBJECT;
                                let nodeValue;
                                if (!isLast) {
                                    nodeValue = nodeType === StorageNode_1.VALUE_TYPES.OBJECT ? {} : [];
                                }
                                else {
                                    nodeValue = otherNode.value;
                                    if (nodeType === StorageNode_1.VALUE_TYPES.ARRAY) {
                                        nodeValue = objectToArray(nodeValue);
                                    }
                                }
                                if (key in parent) {
                                    // Merge with parent
                                    const mergePossible = typeof parent[key] === typeof nodeValue && [StorageNode_1.VALUE_TYPES.OBJECT, StorageNode_1.VALUE_TYPES.ARRAY].includes(nodeType);
                                    if (!mergePossible) {
                    ...
                    if (options.exclude) {
                        ...
                        options.exclude.forEach((path) => {
                            const checkKeys = PathInfo_1.PathInfo.getPathKeys(path);
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (!node && path !== '') {
                    ...
                    const lockPath = yield transaction.moveToParentPath(pathInfo.parentPath);
                    (0, assert_1.assert)(lockPath === pathInfo.parentPath, `transaction.moveToParentPath() did not move to the right parent path of "${path}"`);
                    const parent = yield this._readNode(pathInfo.parentPath, { transaction });
                    if (parent && [StorageNode_1.VALUE_TYPES.OBJECT, StorageNode_1.VALUE_TYPES.ARRAY].includes(parent.type) && pathInfo.key in parent.value) {
                ...
                if (options.include_child_count) {
                    info.childCount = 0;
                    if ([StorageNode_1.VALUE_TYPES.OBJECT, StorageNode_1.VALUE_TYPES.ARRAY].includes(info.valueType) && info.address) {
                        // Get number of children
                        info.childCount = node.value ? Object.keys(node.value).length : 0;
                        info.childCount += yield transaction.getChildCount(path);
                    }
                }
                if (!options.transaction) {
                    // transaction was created by us, commit
    ...
    setNode(path, value, options = { suppress_events: false, context: null }) {
        return __awaiter(this, void 0, void 0, function* () {
            if (this.settings.readOnly) {
                throw new Error(`Database is opened in read-only mode`);
            }
            const pathInfo = PathInfo_1.PathInfo.get(path);
            const transaction = options.transaction || (yield this._customImplementation.getTransaction({ path, write: true }));
            try {
                if (path === '') {
                    ...
                    yield this._writeNodeWithTracking('', value, {
                        merge: false,
                        transaction,
                    ...
                    const info = yield this.getNodeInfo(path, { transaction });
                    ...
                    if (info.address && info.address.path === path && value !== null && !this.valueFitsInline(value)) {
                        // Overwrite node
                        yield this._writeNodeWithTracking(path, value, {
                            merge: false,
                            transaction,
                            suppress_events: options.suppress_events,
                            context: options.context,
                        ...
                        const lockPath = yield transaction.moveToParentPath(pathInfo.parentPath);
                        (0, assert_1.assert)(lockPath === pathInfo.parentPath, `transaction.moveToParentPath() did not move to the right parent path of "${path}"`);
                        yield this._writeNodeWithTracking(pathInfo.parentPath, { [pathInfo.key]: value }, { merge: true, transaction, suppress_events: options.suppress_events, context: options.context });
                    }
                }
                else {
                    ...
                    const lockPath = yield transaction.moveToParentPath(pathInfo.parentPath);
                    (0, assert_1.assert)(lockPath === pathInfo.parentPath, `transaction.moveToParentPath() did not move to the right parent path of "${path}"`);
                    yield this.updateNode(pathInfo.parentPath, { [pathInfo.key]: value }, { transaction, suppress_events: options.suppress_events, context: options.context });
                }
                if (!options.transaction) {
                    // transaction was created by us, commit
                    yield transaction.commit();
    ...
    updateNode(path, updates, options = { suppress_events: false, context: null }) {
        return __awaiter(this, void 0, void 0, function* () {
            if (this.settings.readOnly) {
                throw new Error(`Database is opened in read-only mode`);
            }
            ...
            try {
                ...
                if (nodeInfo.exists && nodeInfo.address && nodeInfo.address.path === path) {
                    ...
                    const lockPath = yield transaction.moveToParentPath(pathInfo.parentPath);
                    (0, assert_1.assert)(lockPath === pathInfo.parentPath, `transaction.moveToParentPath() did not move to the right parent path of "${path}"`);
                    yield this._writeNodeWithTracking(pathInfo.parentPath, { [pathInfo.key]: updates }, { transaction, merge: true, suppress_events: options.suppress_events, context: options.context });
                }
                else {
                    // The node does not exist, it's parent doesn't have it either. Update the parent instead
                    const lockPath = yield transaction.moveToParentPath(pathInfo.parentPath);
                    (0, assert_1.assert)(lockPath === pathInfo.parentPath, `transaction.moveToParentPath() did not move to the right parent path of "${path}"`);
                    yield this.updateNode(pathInfo.parentPath, { [pathInfo.key]: updates }, { transaction, suppress_events: options.suppress_events, context: options.context });
                }
                if (!options.transaction) {
                    // transaction was created by us, commit
                    yield transaction.commit();
