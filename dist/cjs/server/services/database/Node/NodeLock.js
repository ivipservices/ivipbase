...
class NodeLocker {
    ...
    _allowLock(path, tid, forWriting) {
        ...
        const conflict = this._locks.find((otherLock) => {
            return otherLock.tid !== tid && otherLock.state === exports.LOCK_STATE.LOCKED && (forWriting || otherLock.forWriting);
        });
    ...
    _processLockQueue() {
        if (this._quit) {
            ...
            this._locks.filter((lock) => lock.state === exports.LOCK_STATE.PENDING).forEach((lock) => this._rejectLock(lock, quitError));
    ...
    async lock(path, tid, forWriting = true, comment = "", options = { withPriority: false, noTimeout: false }) {
        ...
        if (proceed) {
            ...
            assert(lock.state === exports.LOCK_STATE.PENDING);
...
class NodeLock {
    ...
    async moveToParent() {
        const parentPath = ivipbase_core_1.PathInfo.get(this.path).parentPath; //getPathInfo(this.path).parent;
...
//# sourceMappingURL=NodeLock.js.map