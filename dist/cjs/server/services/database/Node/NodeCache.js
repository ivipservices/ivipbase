...
class NodeCacheEntry {
    constructor(nodeInfo) {
        ...
        this.pathInfo = ivipbase_core_1.PathInfo.get(nodeInfo.path);
...
class NodeCache {
     ...
     */
    update(nodeInfo) {
        if (!(nodeInfo instanceof NodeInfo_1.default)) {
            ...
            this._cache.forEach((entry, cachedPath) => {
                if (pathInfo.isAncestorOf(entry.pathInfo)) {
                    if (typeof recursive === "object") {
                        ...
                        switch (action) {
                            case "delete":
                                this.update(new NodeInfo_1.default({ path: cachedPath, exists: false }));
                                break;
                            case "invalidate":
    ...
    delete(path) {
        ...
        this.update(new NodeInfo_1.default({ path, exists: false }));
        this._cache.forEach((entry, cachedPath) => {
